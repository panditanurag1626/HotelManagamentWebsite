...
export default function Register() {
  ...
  // Add booking page class to body
  useEffect(() => {
  ...
  const validate = () => {
    ...
    if (!formData.unum) err.unum = "Phone number is required";
    if (!formData.uemail) err.uemail = "Email is required";
    if (!formData.upass) err.upass = "Password is required";
    if (!formData.ucpass) err.ucpass = "Confirm password is required";

    if (formData.upass !== formData.ucpass) {
      err.ucpass = "Passwords do not match";
    }

    if (!formData.checkIn) err.checkIn = "Check-in date required";
    if (!formData.checkOut) err.checkOut = "Check-out date required";
  ...
  const handleSubmit = (e) => {
    ...
    alert("Registration & Booking Successful");
    navigate("/login");
  };
  ...
  return (
    <div className="container" style={{ maxWidth: "420px", margin: "40px auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label>Phone</label>
        <input type="text" name="unum" value={formData.unum} onChange={handleChange} />
        {errors.unum && <p className="text-danger">{errors.unum}</p>}

        <label>Email</label>
        <input type="email" name="uemail" value={formData.uemail} onChange={handleChange} />
        {errors.uemail && <p className="text-danger">{errors.uemail}</p>}

        <label>Password</label>
        <input type="password" name="upass" value={formData.upass} onChange={handleChange} />
        {errors.upass && <p className="text-danger">{errors.upass}</p>}

        <label>Confirm Password</label>
        <input type="password" name="ucpass" value={formData.ucpass} onChange={handleChange} />
        ...
        <label >Role</label>
        <select name="role" value={formData.role} onChange={handleChange} style={{background:"transparent"}}> 
          <option value="user" style={{background:"transparent"}}>User</option>
